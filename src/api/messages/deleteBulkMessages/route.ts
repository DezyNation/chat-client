// pages/api/channels/deleteBulkMessages.ts
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'DELETE') {
    try {
      ...
      // Get the message IDs from the request body (an array of message IDs)
      const messageIds = req.body.messageIds;

      // Send a DELETE request using the userAxiosInstance to delete multiple messages
      const response = await userAxiosInstance.delete(`/channels/${targetChannelId}/messages/bulk`, {
        data: { messageIds },
      });
