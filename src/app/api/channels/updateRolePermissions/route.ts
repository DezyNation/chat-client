// pages/api/channels/updateRolePermissions.ts
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'PUT') {
    try {
      // Get the target group ID and role ID from the query parameters
      ...
      // Send a PUT request using the userAxiosInstance to modify the role's permissions
      const response = await userAxiosInstance.put(`/channels/${targetGroupId}/permissions/${roleId}`, permissions);
